            ...
            <div class="m-6 p-8 bg-slate-700 rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.reservations.update', $reservation->id) }}">
                    ...
                    <div>
                        ...
                        <input type="number" id="guest_number" min="0" max="8" step="1" name="guest_number" value="{{ $reservation->guest_number }}"
                            class="block w-1/2 p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-400 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    @error('guest_number')
                        <div class="text-sm text-red-400">{{ $message }}</div>
                        ...
                        <div class="mt-1">
                            <select id=table_id" name="table_id" class="form-multiselect block w-1/2 mt-1 rounded-lg">
                                @foreach ($tables as $table)
                                    <option value="{{ $table->id }}" @selected($table->id == $reservation->table_id)>{{ $table->name }} ({{ $table->guest_number }} Seats)</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <button type="submit"
                        class="px-6 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-sm text-white">Update Reservation</button>
                </form>
